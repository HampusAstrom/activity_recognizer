...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function out_data=GPSaidedINS2(in_data,settings)
...
for k=2:N
  ...
  %  y = [y; in_data.SPEEDOMETER.speed(:,ctr_speed_data); zeros(2,1)];
%---> y = ...put in code here for tasks 2-4, 
%---> adding SPEEDOMETER data and holonomic constraints
   
      
    H = [eye(3) zeros(3,12)];
 %   H(4:6,4:6) = get_Rb2p()*q2dcm(x_h(7:10))';
   
    
    R = diag([settings.sigma_gps^2*ones(3,1)]);
   % R = diag([settings.sigma_gps^2*ones(3,1);settings.sigma_speed^2; settings.sigma_non_holonomic^2*ones(2,1)]);
...
out_data.x_h = real(out_data.x_h);
end
